...
// Function to fill address book with dummy contacts
void populateAddressBook(AddressBook* addressBook)
{
    int numDummyContacts = sizeof(dummyContacts) / sizeof(dummyContacts[0]);//Declare variable to calculate how many dummy contacts exist
    // Loop for checking dummy contacts
    for (int i = 0; i < numDummyContacts && addressBook->contactCount < MAX_CONTACTS; ++i) {
        addressBook->contacts[addressBook->contactCount++] = dummyContacts[i];//Copy each dummy contact into the address book and increase contact count
    }
}